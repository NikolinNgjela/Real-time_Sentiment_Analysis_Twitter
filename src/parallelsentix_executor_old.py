...
class Worker(Callable):
	...
	def __init__(self, sentiworddic, amplifiers, decrementers, tid, tdata):
		self.started = None
		self.completed = None
		self.result = None
...
class MasterCaller:
	...
	def classify_sent(self,texts):
		...
		self.workers = [Worker(self.sentiworddic,self.amplifiers,self.decrementers,i,self.tdatas[i]) for i in range(self.k)]
